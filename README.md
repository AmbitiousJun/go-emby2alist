...
- Alist 网盘转码直链播放
  ...
  > 局限：
  >
  > 视频本身的内封字幕会丢失，尽管 Alist 接口也返回了转码字幕直链地址，但是将该字幕信息嵌入视频的 PlaybackInfo 后，Emby 还是无法将其挂载上
  >
  > 不过好在有的客户端是能将原画的**部分**内封字幕挂载到转码资源上的，比如 Fileball：
  >
  > 只需要在开始播放之前，在字幕一栏选中自己想要的字幕，再点击播放后就能挂载上去
  >
  > ![](assets/2024-09-21-10-12-21.png)
...
| `Emby for Android`               | 使用安卓 TV 测试，基本操作正常                               |
| `Emby for AndroidTV`             | 基本操作正常，使用自带播放器播放原画时无法频繁跳进度，会被限制，但可以通过播放转码直链解决 |
| `Fileball`                       | 基本操作正常，目前为止测试过的兼容性最好的客户端，可惜只支持 IOS |
