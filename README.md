...
```shell
git clone --branch v1.1.0 --depth 1 https://ghproxy.cc/https://github.com/AmbitiousJun/go-emby2alist
cd go-emby2alist
```
...
参考[示例配置](https://github.com/AmbitiousJun/go-emby2alist/blob/v1.1.0/config-example.yml)，配置好自己的服务器信息，保存并命名为 `config.yml`
...
services:
  go-emby2alist:
    image: ambitiousjun/go-emby2alist:v1.1.0
    environment:
      - TZ=Asia/Shanghai
      - GIN_MODE=release
